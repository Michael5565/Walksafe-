...
// GET /api/auth/verify-email — Validate token, mark email verified, redirect
app.get("/auth/verify-email", async (c) => {
  ...
  if (!record) {
    return c.redirect("https://app.getwalksafe.co.uk/?verify=failed");
  }
  ...
  if (verified) {
    return c.redirect("https://app.getwalksafe.co.uk/?verify=success");
  } else {
    return c.redirect("https://app.getwalksafe.co.uk/?verify=error");
  }
});
